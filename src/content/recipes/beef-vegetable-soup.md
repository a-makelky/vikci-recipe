---
id: scan-only-beef-vegetable-soup
title: Beef Vegetable Soup
summary: Scan is available. Transcription is still pending.
ingredients:
  - Transcription pending. Read from the original scan.
instructions:
  - Transcription pending. Read from the original scan.
notes:
  - Published as a scan-first archive entry for Mother's Day.
  - OCR and structured cleanup can be added later.
source_name: Vicki
source_family: Makelky
course: soup
proteins: []
cuisine: unknown
dessert: false
tags:
  - soups
  - scan-first
  - needs transcription
card_type: mixed
ocr_confidence: low
review_status: needs_transcription
scan_assets:
  - path: /scans/beef-vegetable-soup/front-preview.jpg
    label: Front preview
    type: image
    role: preview
  - path: /scans/beef-vegetable-soup/back-preview.jpg
    label: Back preview
    type: image
    role: preview
  - path: /scans/beef-vegetable-soup/front-original.jpg
    label: Front scan
    type: image
    role: original
  - path: /scans/beef-vegetable-soup/back-original.jpg
    label: Back scan
    type: image
    role: original
---

This recipe is live as an image-first archive entry.

The original card scan is the source of truth until transcription is reviewed.
