---
id: scan-only-quick-cinnamon-rolls
title: Quick Cinnamon Rolls
summary: Scan is available. Transcription is still pending.
ingredients:
  - Transcription pending. Read from the original scan.
instructions:
  - Transcription pending. Read from the original scan.
notes:
  - Published as a scan-first archive entry for Mother's Day.
  - OCR and structured cleanup can be added later.
source_name: Vicki
source_family: Makelky
course: bread
proteins: []
cuisine: unknown
dessert: false
tags:
  - bread
  - scan-first
  - needs transcription
card_type: mixed
ocr_confidence: low
review_status: needs_transcription
scan_assets:
  - path: /scans/quick-cinnamon-rolls/front-preview.jpg
    label: Front preview
    type: image
    role: preview
  - path: /scans/quick-cinnamon-rolls/back-preview.jpg
    label: Back preview
    type: image
    role: preview
  - path: /scans/quick-cinnamon-rolls/front-original.jpg
    label: Front scan
    type: image
    role: original
  - path: /scans/quick-cinnamon-rolls/back-original.jpg
    label: Back scan
    type: image
    role: original
---

This recipe is live as an image-first archive entry.

The original card scan is the source of truth until transcription is reviewed.
